...
def getFieldsArrStr(a):
  ...
  if a.read_data.has_key("props"):
    for i,v in enumerate(a.read_data["props"]):
      ...
      if v.has_key("size"):
        ...
        arr.append("public "+t+" "+v["name"]+" = "+str(v["value"]))
      else:
        arr.append("public "+t+" "+v["name"])
  ...
  for v in a.read_data["args"]:
    ...
    v["type"] = t
    arr.append("public "+v["type"]+" "+v["name"])

  for i,v in enumerate(a.read_data["connection"]["writeTo"]):
    arr.append("public "+"writer w"+str(i))

  for i,v in enumerate(a.read_data["connection"]["readFrom"]):
    arr.append("public "+"reader r"+str(i))
  if len(a.read_data["connection"]["readFrom"]) > 1:
    arr.append("public "+"reader rSelect")
    arr.append("public "+"selector readersSelector")
  return arr
